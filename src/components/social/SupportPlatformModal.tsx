...
interface SupportPlatformModalProps {
  redirectTo?: string;
  onClose: () => void;
}

export function SupportPlatformModal({ redirectTo = "/", onClose }: SupportPlatformModalProps) {
  ...
  const handleDonate = async () => {
    ...
    try {
      ...
      const response = await fetch("/api/platform/donate", {
        method: "POST",
        headers: {
        ...
        body: JSON.stringify({
          amountUsd,
          redirectTo: `${redirectTo}${redirectTo.includes("?") ? "&" : "?"}platform_donated=true`,
        }),
      });
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/50 p-4">
      <div className="relative w-full max-w-md rounded-3xl border border-sidebar bg-paper p-6 shadow-2xl">
        ...
        <h2 className="font-serif text-2xl font-bold text-ink">Invita un café al Imperio</h2>
        <p className="mt-2 text-sm text-muted">
          Tu aporte ayuda a {BRAND_NAME} a seguir creciendo: infraestructura, mejoras del lector y
          difusión de nuevas voces.
        </p>
